...
void *func_1(void *args){
    ...
    while(true){
        ...
        if(datalen > shm_size){
            ...
            shmid = get_sh(MAX_SHM_STRIP*global.nCameraID+t_index, datalen);
            shm_size = datalen;
            if(0 > shmid){
        ...
        while(true){
            if(0 == t_ptr->flag){
                memset(t_ptr->value, 0, 256);
                strncpy(t_ptr->value, (char*)&datalen, sizeof(datalen));//要传的数据的大小
                ...
                //使用用例
                size_t result_len = ((size_t*)((unsigned long)(t_ptr->value) + sizeof(int)))[0];
                //cout<<"result_len: "<<result_len<<endl;
                ((THREAD_DATA*)args)->ptr_len = result_len;
                ((THREAD_DATA*)args)->ptr = new unsigned char(result_len);
                ((THREAD_DATA*)args)->occur = ((int*)t_ptr->value)[0] & 0xff;//我的用例1是图片，2是boxes,你可以自己合理使用
                ((THREAD_DATA*)args)->type = NOT_EXIST_RANGE;
...
void send_to_remote(THREAD_DATA* d_thread){
    ...
    for(int index=0; index<MAX_THREAD; index++){
        if(d_thread->occur){
            ...
            if(d_thread->ptr_len){
                switch(d_thread->occur){//可定义返回的大数据自己的使用办法
                case 1://ptr中存的是一幅画
                    break;
                case 2://ptr中存的是boxes
                    break;
                ...
                if(d_thread->ptr) delete(d_thread->ptr);
            }
        }
        d_thread++;
    }
