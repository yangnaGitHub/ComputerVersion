...
def draw_bboxes_video(image, bboxes, confidences, categories, all_categories, message=None):
    for box, score, category in zip(bboxes, confidences, categories):
        x_coord, y_coord, width, height = box
        ...
        cv2.rectangle(image, \
            (left, top), (right, bottom), (255, 0, 0), 1)
...
def reshape_image(img, inp_resolution):
    img = cv2.resize(img, inp_resolution,interpolation=cv2.INTER_NEAREST)
    img = cv2.cvtColor(img, cv2.COLOR_BGRA2RGB)
    # Convert 8-bit integer to 32-bit floating point
    img = img.astype(np.float32)
    # Convert HWC to CHW
    img_ = img[:,:,::-1].transpose((2,0,1)).copy() #np.transpose(img, [2, 0, 1])
    # Convert CHW to NCHW
    img_ = np.expand_dims(img_, axis=0)/255.0
    # Convert to row-major
    img = np.array(img_, dtype=np.float32, order='C')
    return img_
