...
def dynamic_write_results(prediction, confidence, num_classes, nms=True, nms_conf=0.4, selected_class_num=0):
    prediction_bak = prediction.clone()
    dets = write_results(prediction.clone(), confidence, num_classes, nms, nms_conf, selected_class_num)
    if isinstance(dets, int):
        return dets

    if dets.shape[0] > 100:
        nms_conf -= 0.05
        dets = write_results(prediction_bak.clone(), confidence, num_classes, nms, nms_conf, selected_class_num)

    return dets


def write_results(prediction, confidence, num_classes, nms=True, nms_conf=0.4, selected_class_num=0):
    ...
    for ind in range(batch_size):
        ...
        for cls in img_classes:
            if cls != selected_class_num: #0 is the person
                continue
