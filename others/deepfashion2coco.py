# -*- coding: utf-8 -*-
"""
Created on Sat Sep  5 11:06:08 2020
...
rootpath = '/home/yangna/deepfashion/val'
import os
import glob
images = os.path.join(rootpath, 'image')
annos = os.path.join(rootpath, 'annos')
annosfiles = glob.glob(os.path.join(annos, '*.json'))
for annosfile in annosfiles:
    try:
        basename = os.path.basename(annosfile)
        image = cv2.imread(os.path.join(images, basename[:-4]+'jpg'))
        height, width = image.shape[:2]
        with open(annosfile, 'r') as f:
            temp = json.loads(f.read())
    except:
        print(basename, os.path.join(images, basename[:-4]+'jpg'))
    basename = '1'+basename
    image = cv2.imwrite(os.path.join(images, 'rename', basename[:-4]+'jpg'), image)
    pair_id = temp['pair_id']

    dataset['images'].append({
        'coco_url': '',
        'date_captured': '',
        'file_name': basename[:-4]+'jpg',
        'flickr_url': '',
        'id': int(basename[:-5]),
        'license': 0,
        'width': width,
...
json_name = os.path.join(rootpath, 'deepfashion2.json')
